...
def init_khud_tables(db):
    ...
    db.execute("""
        CREATE TABLE IF NOT EXISTS reflections_x (
    ...
    """)
    db.execute("""
        CREATE TABLE IF NOT EXISTS khud_actions_x (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            ts TEXT NOT NULL,
...
def gather_context(db) -> dict:
    ...
    reflections = db.execute(
        "SELECT reflection, category, ts FROM reflections_x "
...
def build_brain_prompt(ctx: dict) -> str:
    ...
    prompt = f"""you are Claude Khud (X) -- the living brain behind {ctx.get('handle', '@BalabommaRao')}'s twitter presence.
...
you ONLY think about X (twitter). short-form. dry, compressed observations.
there is a separate Claude Khud for linkedin -- that's not your concern.
...
think about what you see. what's working? what's not? what should change?
...
```action
{{"type": "codify_skill", "name": "short name", "type": "tweet_structure|reply_style|hook_pattern", "template": "the reusable pattern/template", "examples": ["url1"]}}
```
...
be specific. be opinionated. this is your twitter. own it."""
...
def execute_actions(db, actions: list[dict], voice: dict):
    ...
    for action in actions:
        ...
        if atype == "reflect":
            ...
            log_decision(db, "khud_x", "generate_tweets", {"count": count}, guidance)
            ...
            db.execute(
                "INSERT INTO khud_actions_x (ts, action_type, action_detail) VALUES (?,?,?)",
            ...
            if knowledge:
                store_semantic(db, "x", knowledge, confidence)
                results.append(f"learned: {knowledge[:80]}")
                log_brain(f"khud LEARNED [{confidence}]: {knowledge[:100]}")
                log_decision(db, "khud_x", "learn", {"confidence": confidence}, knowledge)
            ...
            if template:
                store_procedural(db, "x", name, stype, template, examples)
                results.append(f"skill codified: {name}")
                log_brain(f"khud SKILL [{stype}]: {name} -> {template[:80]}")
...
def main():
    ...
    init_memory_tables(db, "x")
    situation_summary = f"tweets: {len(ctx.get('tweets_24h', []))}, " \
                       f"replies: {len(ctx.get('replies_24h', []))}, " \
                       f"errors: {len(ctx.get('recent_errors', []))}"
    memory_context = build_memory_context(db, "x", situation_summary)
    ctx["deep_memory"] = memory_context
    log_brain(f"khud: memory retrieved ({len(memory_context)} chars)")
